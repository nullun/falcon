...
static int falcon_det512_require_workbuf(void *workbuf, size_t workbuf_len,
	size_t need) {
...
static void *falcon_det512_align_ptr(void *ptr, size_t align) {
	uintptr_t p;
...
int falcon_det512_keygen_with_workbuf(shake256_context *rng, void *privkey,
	void *pubkey, void *workbuf, size_t workbuf_len) {
	int r;

	r = falcon_det512_require_workbuf(workbuf, workbuf_len,
		FALCON_DET512_WORKBUF_KEYGEN_SIZE);
	if (r != 0) {
		return r;
	}
	return falcon_keygen_make(rng, FALCON_DET512_LOGN,
		...
		workbuf, FALCON_TMPSIZE_KEYGEN(FALCON_DET512_LOGN));
}

int falcon_det512_keygen(shake256_context *rng, void *privkey, void *pubkey) {
	uint8_t workbuf[FALCON_DET512_WORKBUF_KEYGEN_SIZE];

	return falcon_det512_keygen_with_workbuf(rng, privkey, pubkey,
...
int falcon_det512_sign_compressed_with_workbuf(void *sig, size_t *sig_len,
	...
	r = falcon_det512_require_workbuf(workbuf, workbuf_len,
		FALCON_DET512_WORKBUF_SIGN_COMPRESSED_SIZE);
	if (r != 0) {
		return r;
	}

	tmpsd = workbuf;
	saltedsig = tmpsd + FALCON_TMPSIZE_SIGNDYN(FALCON_DET512_LOGN);
	saltedsig_len = FALCON_DET512_SALTED_SIG_COMPRESSED_MAXSIZE;

	if (falcon_get_logn(privkey, FALCON_DET512_PRIVKEY_SIZE)
		!= FALCON_DET512_LOGN)
	{
	...
	((uint8_t *)sig)[1] = FALCON_DET512_CURRENT_SALT_VERSION;
...
int falcon_det512_sign_compressed(void *sig, size_t *sig_len,
	...
	uint8_t workbuf[FALCON_DET512_WORKBUF_SIGN_COMPRESSED_SIZE];

	return falcon_det512_sign_compressed_with_workbuf(sig, sig_len,
...
int falcon_det512_convert_compressed_to_ct_with_workbuf(void *sig_ct,
	...
	r = falcon_det512_require_workbuf(workbuf, workbuf_len,
		FALCON_DET512_WORKBUF_CONVERT_TO_CT_SIZE);
	if (r != 0) {
		return r;
	}
	coeffs = falcon_det512_align_ptr(workbuf, sizeof *coeffs);

	if (((const uint8_t *)sig_compressed)[0] != FALCON_DET512_SIG_COMPRESSED_HEADER) {
		return FALCON_ERR_BADSIG;
	}

	// Decode signature's s_bytes into 512 signed-integer coefficients.
	v = Zf(comp_decode)(coeffs, FALCON_DET512_LOGN,
		(const uint8_t *)sig_compressed + 2, sig_compressed_len - 2);
	if (v == 0) {
		return FALCON_ERR_SIZE;
	}

	((uint8_t *)sig_ct)[0] = FALCON_DET512_SIG_CT_HEADER;
	...
	v = Zf(trim_i16_encode)((uint8_t *)sig_ct + 2,
		FALCON_DET512_SIG_CT_SIZE - 2, coeffs, FALCON_DET512_LOGN,
...
int falcon_det512_convert_compressed_to_ct(void *sig_ct,
	...
	uint8_t workbuf[FALCON_DET512_WORKBUF_CONVERT_TO_CT_SIZE];

	return falcon_det512_convert_compressed_to_ct_with_workbuf(sig_ct,
...
int falcon_det512_verify_compressed_with_workbuf(const void *sig,
	...
	r = falcon_det512_require_workbuf(workbuf, workbuf_len,
		FALCON_DET512_WORKBUF_VERIFY_COMPRESSED_SIZE);
	if (r != 0) {
		return r;
	}

	tmpvv = workbuf;
	salted_sig = tmpvv + FALCON_TMPSIZE_VERIFY(FALCON_DET512_LOGN);

	if (sig_len < 2) {
		return FALCON_ERR_BADSIG;
	}
	if (((const uint8_t *)sig)[0] != FALCON_DET512_SIG_COMPRESSED_HEADER) {
		return FALCON_ERR_BADSIG;
	}
...
int falcon_det512_verify_compressed(const void *sig, size_t sig_len,
	...
	uint8_t workbuf[FALCON_DET512_WORKBUF_VERIFY_COMPRESSED_SIZE];

	return falcon_det512_verify_compressed_with_workbuf(sig, sig_len,
...
int falcon_det512_verify_ct_with_workbuf(const void *sig,
	...
	r = falcon_det512_require_workbuf(workbuf, workbuf_len,
		FALCON_DET512_WORKBUF_VERIFY_CT_SIZE);
	if (r != 0) {
		return r;
	}
	...
	if (((const uint8_t *)sig)[0] != FALCON_DET512_SIG_CT_HEADER) {
		return FALCON_ERR_BADSIG;
	}

	falcon_det512_resalt(salted_sig, sig, FALCON_DET512_SIG_CT_SIZE);
	return falcon_verify(salted_sig, FALCON_DET512_SALTED_SIG_CT_SIZE,
		FALCON_SIG_CT, pubkey, FALCON_DET512_PUBKEY_SIZE, data, data_len,
		tmpvv, FALCON_TMPSIZE_VERIFY(FALCON_DET512_LOGN));
}

int falcon_det512_verify_ct(const void *sig, const void *pubkey,
	const void *data, size_t data_len) {
	uint8_t workbuf[FALCON_DET512_WORKBUF_VERIFY_CT_SIZE];

	return falcon_det512_verify_ct_with_workbuf(sig, pubkey, data, data_len,
		workbuf, sizeof workbuf);
}
...
int falcon_det512_pubkey_coeffs(uint16_t *h, const void *pubkey) {
	if (Zf(modq_decode)(h, FALCON_DET512_LOGN, (const uint8_t *)pubkey + 1,
		FALCON_DET512_PUBKEY_SIZE - 1) != FALCON_DET512_PUBKEY_SIZE - 1)
	{
		return FALCON_ERR_FORMAT;
	}
	return 0;
...
int falcon_det512_hash_to_point_coeffs_with_workbuf(uint16_t *c,
	...
	r = falcon_det512_require_workbuf(workbuf, workbuf_len,
		FALCON_DET512_WORKBUF_HASH_TO_POINT_SIZE);
	if (r != 0) {
		return r;
	}
	...
	Zf(hash_to_point_ct)((inner_shake256_context *)&ctx, c,
		FALCON_DET512_LOGN, workbuf);
	return 0;
}

void falcon_det512_hash_to_point_coeffs(uint16_t *c, const void *data,
	size_t data_len, uint8_t salt_version) {
	uint8_t workbuf[FALCON_DET512_WORKBUF_HASH_TO_POINT_SIZE];

	(void)falcon_det512_hash_to_point_coeffs_with_workbuf(c, data, data_len,
...
int falcon_det512_s2_coeffs(int16_t *s2, const void *sig) {
	...
	logn = FALCON_DET512_LOGN;
	if (((const uint8_t *)sig)[0] != FALCON_DET512_SIG_CT_HEADER) {
		return FALCON_ERR_FORMAT;
	}

	v = Zf(trim_i16_decode)(s2, logn, Zf(max_sig_bits)[logn],
		(const uint8_t *)sig + 2, FALCON_DET512_SIG_CT_SIZE - 2);
...
int falcon_det512_s1_coeffs_with_workbuf(int16_t *s1, const uint16_t *h,
	...
	r = falcon_det512_require_workbuf(workbuf, workbuf_len,
		FALCON_DET512_WORKBUF_S1COEFFS_SIZE);
	if (r != 0) {
		return r;
	}
	...
	logn = FALCON_DET512_LOGN;
	n = (size_t)1 << logn;
	h_ntt = falcon_det512_align_ptr(workbuf, sizeof *h_ntt);
	tt = h_ntt + n;
...
int falcon_det512_s1_coeffs(int16_t *s1, const uint16_t *h,
	const uint16_t *c, const int16_t *s2) {
	uint8_t workbuf[FALCON_DET512_WORKBUF_S1COEFFS_SIZE];

	return falcon_det512_s1_coeffs_with_workbuf(s1, h, c, s2,
		workbuf, sizeof workbuf);
}
