...
// enable in order to generate KATs (pipe output to test_deterministic512_kat.h)
...
static uint8_t sigs_ct[NUM_KATS][FALCON_DET512_SIG_CT_SIZE];
...
void test_inner(size_t data_len) {
	...
	memset(privkey, 0, FALCON_DET512_PRIVKEY_SIZE);
	memset(pubkey, 0, FALCON_DET512_PUBKEY_SIZE);
	...
	int r = falcon_det512_keygen(&key_rng, privkey, pubkey);
	if (r != 0) {
		fprintf(stderr, "keygen (data_len=%zu) failed: %d\n", data_len, r);
		exit(EXIT_FAILURE);
	}

	memset(sig, 0, FALCON_DET512_SIG_COMPRESSED_MAXSIZE);
	r = falcon_det512_sign_compressed(sig, &sig_len, privkey, data, data_len);
	if (r != 0) {
		fprintf(stderr, "sign_compressed (data_len=%zu) failed: %d\n", data_len, r);
		exit(EXIT_FAILURE);
	}

	int v = falcon_det512_get_salt_version(sig);
	if (v != FALCON_DET512_CURRENT_SALT_VERSION) {
		fprintf(stderr, "unexpected salt version: %d", v);
		exit(EXIT_FAILURE);
	}

	r = falcon_det512_verify_compressed(sig, sig_len, pubkey, data, data_len);
	if (r != 0) {
		fprintf(stderr, "verify_compressed (data_len=%zu) failed: %d\n", data_len, r);
		exit(EXIT_FAILURE);
	}

	r = falcon_det512_convert_compressed_to_ct(sigs_ct[data_len], sig, sig_len);
	if (r != 0) {
	...
	r = falcon_det512_verify_ct(sigs_ct[data_len], pubkey, data, data_len);
	if (r != 0) {
	...
	size_t elen = hextobin(expected_sig, FALCON_DET512_SIG_COMPRESSED_MAXSIZE, FALCON_DET512_KAT[data_len]);
	if (elen != sig_len) {
...
int main() {
	...
	for (int kat = 0; kat < NUM_KATS_CT; kat++) {
		printf("\t\"");
		for (int i = 0; i < FALCON_DET512_SIG_CT_SIZE; i++) {
	...
	printf("};\n\n");
#else
	uint8_t expected_sig_ct[FALCON_DET512_SIG_CT_SIZE];
	for (int kat = 0; kat < NUM_KATS_CT; kat++) {
		hextobin(expected_sig_ct, FALCON_DET512_SIG_CT_SIZE, FALCON_DET512_KAT_CT[kat]);
		if (memcmp(sigs_ct[kat], expected_sig_ct, FALCON_DET512_SIG_CT_SIZE) != 0) {
